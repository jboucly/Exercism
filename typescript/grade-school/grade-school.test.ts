import GradeSchool, { StudentRoster } from './grade-school'
...
describe('School', () => {
  ...
  it('test perso pour add', () => {
    gradeSchool.addStudent('Blair', 2);
    gradeSchool.addStudent('Toto', 2);
    gradeSchool.addStudent('titi', 1);

    const expectedDb = new StudentRoster(Object.entries({ 1: ['titi'], 2: ['Blair', 'Toto'] }))
  ...
  it('adding more students to the same grade adds them to the roster', () => {
    ...
    const expectedDb = new StudentRoster(Object.entries({ 2: ['Blair', 'James', 'Paul'] }))
    expect(gradeSchool.studentRoster()).toEqual(expectedDb)
  })
  ...
  it('adding students to different grades adds them to the roster', () => {
    ...
    const expectedDb = new StudentRoster(Object.entries({ 3: ['Chelsea'], 7: ['Logan'] }))
    expect(gradeSchool.studentRoster()).toEqual(expectedDb)
  })
  ...
  it('adding a student adds them to the roster for the given grade', () => {
    ...
    const expectedDb = new StudentRoster(Object.entries({ 2: ['Aimee'] }))
    expect(gradeSchool.studentRoster()).toEqual(expectedDb)
  })
  ...
  it('the students names in each grade in the roster are sorted', () => {
    ...
    const expectedSortedStudents = new StudentRoster(Object.entries({
      3: ['Kyle'],
  ...
  it('roster cannot be modified outside of module', () => {
    ...
    result.push('Oops.')
    const expectedDb = new StudentRoster(Object.entries({ 2: ['Aimee'] }))
    let titi = gradeSchool.studentRoster();
    expect(titi).toEqual(expectedDb);
  })
